    ...
    public function updateView() { //update the VIEW based on the users page selection
        if (isset($this->getArray['pageID'])) { //check if a page id is contained in the URL
            switch ($this->getArray['pageID']) {
                ...
                $contentModel = new UnderConstruction($this->user, $this->db, $this->postArray, $this->pageTitle, strtoupper($this->getArray['pageID']), $this->getArray['pageID']);
                $navigationModel = new NavigationManager($this->user, $this->getArray['pageID']);
                // Push models to the controller objects array
                array_push($this->controllerObjects, $navigationModel, $contentModel);
                // Get the page content from the models
                $data = $this->getPageContent($contentModel, $navigationModel);
                // Put the content array into a class property for diagnostic purposes
                $this->viewData = $data;
                // Update the view
                include_once 'views/view_navbar_2_panel.php'; // Load the view
                break; 
                ...
                case "viewJobs":
                    $contentModel = new ViewJobs($this->user, $this->db, $this->postArray, $this->pageTitle, strtoupper($this->getArray['pageID']), $this->getArray['pageID']);
                    $navigationModel = new NavigationManager($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects, $navigationModel, $contentModel);
                    $data = $this->getPageContent($contentModel, $navigationModel);
                    $this->viewData = $data;
                ...
                case "editJob":
                    $contentModel = new EditJob($this->user, $this->db, $this->postArray, $this->pageTitle, strtoupper($this->getArray['pageID']), $this->getArray['pageID']);
                    $navigationModel = new NavigationManager($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects, $navigationModel, $contentModel);
                    $data = $this->getPageContent($contentModel, $navigationModel);
                    $this->viewData = $data;
                ...
                case "addJob":
                    $contentModel = new AddJob($this->user, $this->db, $this->postArray, $this->pageTitle, strtoupper($this->getArray['pageID']), $this->getArray['pageID']);
                    $navigationModel = new NavigationManager($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects, $navigationModel, $contentModel);
                    $data = $this->getPageContent($contentModel, $navigationModel);
                    $this->viewData = $data;
                ...
                case "home":
                    $contentModel = new HomePage($this->user, $this->db, $this->postArray, $this->pageTitle, strtoupper($this->getArray['pageID']), $this->getArray['pageID']);
                    $navigationModel = new NavigationManager($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects, $navigationModel, $contentModel);
                    $data = $this->getPageContent($contentModel, $navigationModel);
                    $this->viewData = $data;
                ...
                // Additional cases for other menu items can be added similarly


                //manage jobs handlers
                case "manageJobs":
                    //create objects to generate view content
                    $contentModel = new ManagerManageJobs($this->user, $this->db, $this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']), $this->getArray['pageID']);
                    $navigationModel = new NavigationManager($this->user, $this->getArray['pageID']);
                    array_push($this->controllerObjects,$navigationModel,$contentModel);
                    $data = $this->getPageContent($contentModel,$navigationModel);  //get the page content from the models                 
                    $this->viewData = $data;  //put the content array into a class property for diagnostic purpose
